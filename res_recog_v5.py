...
net = init_net(layers=LAYERS, batch_size=8, using_gpu=True)

print 'load the mean value...'
mean_bgr = load_mean_bgr()
print 'initialize done!!!'
print 'using time: %.4f s' % (time.clock()-start_ini)
...
def split_to_small_img(img_path, size=224):
	"""将12306验证码分割成小图"""
	res = np.zeros((8, 3, size, size), dtype=np.float32)
	index = 0
	...
	for row in range(0, 2):
		for col in range(0, 4):
			...
			# resize
			short_edge = min(content_img.shape[:2])
			yy = int((content_img.shape[0] - short_edge) / 2)
			xx = int((content_img.shape[1] - short_edge) / 2)
			crop_img = content_img[yy:yy + short_edge, xx:xx + short_edge]
			resized_img = transform.resize(crop_img, (size, size))
			
			# preprocess
			img_p = np.copy(resized_img) * 255.0
			img_p = img_p[:, :, [2, 1, 0]]  # swap channel from RGB to BGR
			img_p -= mean_bgr
			img_p_t = img_p.transpose((2, 0, 1))  # transpose (H,W,C) to (C,H,W)
...
def get_batch_ResNet_vector(img_batch):
	"""获得一个batch的vector"""
	net.blobs['data'].data[...] = img_batch
	assert net.blobs['data'].data.shape[1:] == (3, 224, 224)
	net.forward()

	caffe_fcb = net.blobs['fc1000'].data  # ndarray with shape(batch_size, 1000)
	return caffe_fcb
...
def recog(img_path, index):
	"""识别12306验证码"""
	# split
	small_imgs_arr = split_to_small_img(img_path)
	assert small_imgs_arr.shape[0] == 8
	'''Phase A
	...
	# 1
	# 获得12306小图所代表的vector
	vecs = get_batch_ResNet_vector(small_imgs_arr)
